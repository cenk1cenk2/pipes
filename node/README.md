...
| Flag / Environment |  Description   |  Type    | Required | Default |
|---------------- | --------------- | --------------- |  --------------- |  --------------- |
| `$NODE_RUN_SCRIPT` | package.json script for given command operation. | `string`<br/>`Template(struct { Environment: string, EnvVars: map[string]string })` | `false` | <code></code> |
| `$NODE_RUN_CWD` | Working directory for the given command operation. | `string` | `false` | <code>"."</code> |
