package lint

import (
	"github.com/urfave/cli/v3"
)

const CATEGORY_LINT = "Lint"

//revive:disable:line-length-limit

var Flags = []cli.Flag{
	&cli.BoolFlag{
		Category:    CATEGORY_LINT,
		Name:        "terraform.lint.format-check.enable",
		Sources:     cli.NewValueSourceChain(cli.EnvVar("TERRAFORM_LINT_FORMAT_CHECK_ENABLE")),
		Usage:       "Enable terraform fmt.",
		Required:    false,
		Value:       true,
		Destination: &P.Lint.FormatCheckEnable,
	},
	&cli.StringFlag{
		Category:    CATEGORY_LINT,
		Name:        "terraform.lint.format-check.args",
		Sources:     cli.NewValueSourceChain(cli.EnvVar("TERRAFORM_LINT_FORMAT_CHECK_ARGS")),
		Usage:       "Additional arguments for terraform fmt.",
		Required:    false,
		Value:       "",
		Destination: &P.Lint.FormatCheckArgs,
	},
	&cli.BoolFlag{
		Category:    CATEGORY_LINT,
		Name:        "terraform.lint.validate.enable",
		Sources:     cli.NewValueSourceChain(cli.EnvVar("TERRAFORM_LINT_VALIDATE_ENABLE")),
		Usage:       "Enable terraform validate.",
		Required:    false,
		Value:       true,
		Destination: &P.Lint.ValidateEnable,
	},
	&cli.StringFlag{
		Category:    CATEGORY_LINT,
		Name:        "terraform.lint.validate.args",
		Sources:     cli.NewValueSourceChain(cli.EnvVar("TERRAFORM_LINT_VALIDATE_ARGS")),
		Usage:       "Additional arguments for terraform validate.",
		Required:    false,
		Value:       "",
		Destination: &P.Lint.ValidateArgs,
	},
}
