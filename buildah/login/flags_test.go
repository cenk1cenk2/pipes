...
var _ = Describe("Flags", func() {
	...
	It("reads the registry out of the environment of the pipeline", func() {
		...
		Expect(flag(1).Sources.Chain[0].String()).To(ContainSubstring("BUILDAH_LOGIN_REGISTRY_USERNAME"))
		Expect(flag(2).Sources.Chain[0].String()).To(ContainSubstring("BUILDAH_LOGIN_REGISTRY_PASSWORD"))
	})

	It("files every flag under the container registry category", func() {
		for i := range Flags {
			Expect(flag(i).Category).To(Equal(CATEGORY_CONTAINER_REGISTRY))
		}
	})
