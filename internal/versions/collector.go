	...
	return tl.CreateTask(c.Name, "user").
		...
		Set(func(ctx context.Context, t *Task) error {
			for _, v := range slices.Compact(slices.Clone(c.FromUser)) {
				if err := c.add(ctx, t, out, v); err != nil {
					return err
				}
			}
...
func (c *Collector) FileTask(tl *TaskList, out *[]string) *Task {
	return tl.CreateTask(c.Name, "file").
		...
		Set(func(ctx context.Context, t *Task) error {
			values, err := tagsfile.Parse(t, path.Join(c.FileDir, c.File), c.FileStrict)
...
func (c *Collector) LatestTask(tl *TaskList, out *[]string) *Task {
	return tl.CreateTask(c.Name, "latest").
		...
		Set(func(ctx context.Context, t *Task) error {
			...
			if err := c.add(ctx, t, out, c.LatestValue); err != nil {
				return err
			}

			t.Log.Info(fmt.Sprintf("Will tag as latest since the reference matches: %s -> %s",
				c.LatestValue,
...
func (c *Collector) add(ctx context.Context, t *Task, out *[]string, value string) error {
	processed, err := c.Process(ctx, t, value)
...
func (c *Collector) Process(ctx context.Context, t *Task, value string) (string, error) {
	value, err := c.template(ctx, t, value)

	if err != nil {
		return "", err
	}

	value, err = c.sanitize(t, value)
...
func (c *Collector) template(ctx context.Context, t *Task, value string) (string, error) {
	for _, m := range c.Templates {
		...
		t.Log.Log(ctx, logger.LevelTrace, fmt.Sprintf("Trying to apply template to tag: %s with %v", value, re.String()))
		...
		t.Log.Debug(fmt.Sprintf("Applying template since condition matched for given tag: %s -> %s with %v", value, re.String(), matches))
...
func (c *Collector) sanitize(t *Task, value string) (string, error) {
	for _, m := range c.Sanitize {
		...
		t.Log.Debug(fmt.Sprintf("Trying to sanitize tag: %s with %v", value, re.String()))
		...
		t.Log.Debug(fmt.Sprintf("Sanitizing since condition matched for given tag: %s -> %s with %v", value, re.String(), matches))
