...
func (c *Collector) add(t *Task, out *[]string, value string) error {
	processed, err := c.Process(t, value)
...
func (c *Collector) Process(t *Task, value string) (string, error) {
	value, err := c.template(t, value)
...
func (c *Collector) template(t *Task, value string) (string, error) {
	for _, m := range c.Templates {
		...
		t.Log.Tracef("Trying to apply template to tag: %s with %v", value, re.String())
		...
		t.Log.Debugf("Applying template since condition matched for given tag: %s -> %s with %v", value, re.String(), matches)
...
func (c *Collector) sanitize(t *Task, value string) (string, error) {
	for _, m := range c.Sanitize {
		...
		t.Log.Debugf("Trying to sanitize tag: %s with %v", value, re.String())
		...
		t.Log.Debugf("Sanitizing since condition matched for given tag: %s -> %s with %v", value, re.String(), matches)
